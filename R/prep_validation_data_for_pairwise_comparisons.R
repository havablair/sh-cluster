# goal: prepare data for pairwise comparisons of validation points. We need
# the soil property data to appear alongside the cluster assignments for 
# each value of k. Also need to make sure that for the CIG and NRCS SHI data,
# we only have one validation point per independent experimental unit (so 
# average repeated measures from the same "plot" to get representative
...
shi_val <- read_csv("data/validation_data/NRCS-SHI/shi_site_allvar_validation_20221116.csv") %>% 
...
cig_kssl_val <- read_csv("data/validation_data/cig_ncss-kssl_allvar_validation_20221230.csv")
...
write_csv(val_soil_prop_df, "data/validation_data_clusters_and_soil_props.csv")
