...
pca_comps <- read_csv("data/pca_welch_pairwise_results_all.csv") %>% 
  mutate(soil_var = case_when(
    str_detect(soil_var, "clay") ~ "Clay",
    str_detect(soil_var, "om") ~ "OM",
    str_detect(soil_var, "cec7") ~ "CEC",
    str_detect(soil_var, "caco3") ~ "CaCO3",
    str_detect(soil_var, "db") ~ "BD",
    str_detect(soil_var, "_ec") ~ "EC",
    str_detect(soil_var, "ph") ~ "pH",
    str_detect(soil_var, "lep") ~ "LEP",
    str_detect(soil_var, "ksat") ~ "Ksat",
    str_detect(soil_var, "awc") ~ "AWC"
  ))
...
# note on 2023-03-16, is this even relevant anymore? not presenting total possible
# in current versions of figures
pca_summary <- read_csv("data/pca_welch_pairwise_comparisons_summary.csv") %>% 
...
valid_comps_summary <- pca_summary %>%
  filter(k_opt %in% c("k_4", "k_6", "k_8")) %>%
  select(k_opt, soil_var, perc_valid_comps)

# possible comparisons assuming we had validation data for every cluster
possible_per_var <-
  pca_summary %>% select(k_opt, possible_comps, num_regions) %>%
  distinct() %>%
...
possible_edit <- pca_summary %>% 
  select(k_opt, poss_valid_comps, num_regions) %>%
  distinct() %>% 
  filter(num_regions %in% c(4:9))
...
pairwise_possible_plot <- pca_summary %>%
  mutate(soil_var = case_when(
    ...
    soil_var == "caco3" ~ "CaCO3",
    soil_var == "dbthirdbar" ~ "Bulk Density",
    soil_var == "om_loi" ~ "Organic Matter",
  ...
  geom_line(data = possible_per_var,
            ...
            show.legend = FALSE) + 
  geom_text(aes(x = 9.5, y = 15, label = "Clay"), color = "#00BF7D", size = 3) +
  geom_text(aes(x = 9.6, y = 10, label = "pH"), color = "#E76BF3", size = 3) +
  geom_text(aes(x = 9.7, y = 8, label = "CaCO3"), color = "#A3A500", size = 3) +
  geom_text(aes(x = 9.4, y = 6, label = "BD"), color = "#F8766D",
            angle = 0, size = 3) +
  geom_text(aes(x = 9.5, y = 4, label = "OM"), color = "#00B0F6", size = 3) +
  geom_text(aes(x = 9.4, y = 37.75, label = "n possible"), color = "black", size = 3) +
  theme_minimal() +
...
ggsave("figs/welch_pairwise_comps_by_soil_var.png",
       pairwise_possible_plot,
       width = 5, height = 5, units = "in")

tot_comps <- pca_summary %>%
  filter(num_regions %in% c(4:9)) %>% 
  group_by(k_opt, num_regions) %>% 
...
ggsave("figs/welch_combo_pairwise_plot.png", width = 6, height = 3)
...
gt::gtsave(pairwise_summary_tbl, "figs/welch_pairwise_summary_tbl.png")
...
gt::gtsave(pairwise_summary_tbl_poss, "figs/welch_pairwise_summary_tbl_poss.png")
...
wide_soil_prop_contrasts <- pca_summary %>%
  ...
  select(soil_var, k_opt, n_sig_comps, possible_comps) %>%
  ...
  select(k_opt, possible = possible_comps_claytotal, contains("n_sig_")) %>%
  mutate(k_opt = str_replace(k_opt, "k_", "k=")) %>% 
  gt() %>%
  ...
  tab_source_note(source_note = "Welch & G-H")
  
gtsave(wide_soil_prop_contrasts, "figs/welch_wide_soil_prop_contrasts.png")
