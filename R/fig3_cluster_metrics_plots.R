...
clim_metrics <- read_csv("data/kmeans_cluster_metrics_climate_only.csv")
...
ggsave(filename = "figs/dpi300/mod_metrics_facet_plot.tiff", 
...
# plot climate metrics ----------------------------------------------------


clim_long <- clim_metrics %>% 
  pivot_longer(cols = -c(n_clust),
               names_to = "metric",
               values_to = "value") %>% 
  filter(metric %in% c("tot_wss", "ch_index", "avg_sil", "sse_ratio")) %>% 
  mutate(metric = case_when(
...
clim_ordered <- clim_long %>%
  mutate(metric = fct_relevel(clim_long$metric, c("Ratio WSS to TSS",
                                                 "Calinski-Harabasz Index",
                                                 "Avg. Silhouette Coefficient"
  ))) %>% 
  filter(metric != "Sum of Within-Cluster SSE")

clim_label_pos <- clim_ordered %>% 
  group_by(metric) %>% 
  slice_max(value, n = 1) %>% 
  mutate(lab_xpos = 1,
         lab_ypos = value*0.95,
...
clim_3metrics_plot <- ggplot() +
  geom_line(data = clim_ordered, 
            aes(x = n_clust, y = value)) + 
  geom_point(data = clim_ordered,
             aes(x = n_clust, y = value)) + 
  geom_text(data = clim_label_pos, aes(x = lab_xpos, y = lab_ypos, label = letter_lab)) +
  scale_x_continuous(breaks = seq(2, 6, 1), limits = c(1, NA)) +
  #scale_color_manual(values = c("darkgrey", "black")) +
  facet_wrap(vars(metric), scales = "free", nrow = 1) +
  theme_bw() + 
  xlab("Number of clusters") +
...
ggsave(filename = "figs/dpi300/clim_mod_metrics_facet_plot.tiff", 
       plot = clim_3metrics_plot,
       height = 2.4, 
       width = 7,
       units = "in",
       dpi = 300)  
